...
Нужен был для проверки, будет ли использование FFM структур для хранения типа блока и оффсета в общем сегменте дешевле, чем
использование разных сегментов в GeoDriverFFMLong.

Заметно проигрывает GeoDriverFFMLong кроме комплекс блоков. В основном из-за дополнительных вычислений смещений.

Потенциала особого нет здесь, но достаточно удобно при работе со структурами, фиксированного размера и состава полей.
